	...
	case "PRIVMSG":
		...
		if nil != cmdMatches {
			...
			if CMDCanRun(userName, cmd) {
				...
				case "ban":
					if msg == "!"+cmd {
						bb.Say(fmt.Sprintf("%s banned for life, they suck.", userName))
					} else {
						bb.Say(fmt.Sprintf("%s banned for life, they suck.", getAttributedUser(msg, true)))
					}
				case "unban":
					if msg == "!"+cmd || getAttributedUser(msg, true) == "" {
						bb.Say(fmt.Sprintf("%s are unbanned, they dont suck anymore.", userName))
					} else {
						bb.Say(fmt.Sprintf("%s are unbanned, they dont suck anymore", getAttributedUser(msg, true)))
					}
					//endregion
				...
				case "commands":
					bb.Say(fmt.Sprintf("Available Commands:  %s", getCMDS(userName)))
